...
class WPSTHook
{
    function __construct()
    {
        ...
        add_action('wpst_after_save_shiptrack_post', array($this, 'wpst_save_multiple_package'), 10, 3);
        add_action('wpst_after_save_shiptrack_post', array($this, 'wpst_save_shipment_history'), 20, 3);
        add_action('wpst_after_save_shiptrack_post_send_email', array($this, 'wpst_send_admin_email_notification'), 50, 3);
        add_action('wpst_after_save_shiptrack_post_send_email', array($this, 'wpst_send_client_email_notification'), 60, 3);
        ...
        add_action('shiptrack_info', array($this, 'product_offers'));
    }
    ...
    function wpst_save_shipment_history($shipment_id, $data, $old_status)
    {
        ...
        if (!empty($new_history)) {
            ...
            $new_history['shiptrack_datetime'] = date(wpst_datetime_format());
    ...
    function wpst_send_admin_email_notification($shipment_id, $data, $old_status)
    {
        ...
        $mail_setting = $shiptrack->get_setting_html('email_admin');
    ...
    function wpst_send_client_email_notification($shipment_id, $data, $old_status) {
        ...
        $shortcode_values = $shiptrack->get_shortcode_values($shipment_id);
        $shiptrack_status = sanitize_text_field(get_post_meta($shipment_id, 'shiptrack_status', true));
        $mail_setting = $shiptrack->get_setting_html('email_client');
        $is_enabled = strtoupper($mail_setting['client_enable'] ?? 'YES') == 'YES';
        if (!$is_enabled || $shiptrack_status == $old_status || !in_array($shiptrack_status, wpst_send_client_email_in_status_list())) {
            return false;
        }
    ...
    function wpst_assign_client($shipment_id)
    {
        ...
        if (!wpst_is_user_admin() && !in_array('shiptrack_client', $access_roles_assign)) {
            return false;
        }
        $clients = wpst_get_users_list(array('shiptrack_client'));
        $assigned_client = $shipment_id ? get_post_meta($shipment_id, 'assigned_client', true) : '';
        echo "<div class='card p-0 mb-3'>";
            echo "<h5 class='h4 m-0 card-header'>".__('Assignment', 'shiptrack')."</h5>";
            echo "<div class='card-body'>";
                WPSTForm::gen_field(array(
                    'key' => 'assigned_client',
                    'type' => 'select',
                    'label' => __('Client', 'shiptrack'),
                    'class' => 'selectize w-100',
                    'options' => $clients,
                    'description' => !empty($clients) ? '' : '<span class="text-danger">'.esc_html__('Create shiptrack client user.', 'wpst').'<a href="user-new.php" target="_blank"> '.esc_html('here', 'wpst').' <i class="fa fa-external-link"></i></a></span>',
    ...
    {
        $access_roles_assign = wpst_get_roles_can_assign();
        if (!wpst_is_user_admin() && !in_array('shiptrack_agent', $access_roles_assign)) {
            return false;
        }
        $agents = wpst_get_users_list(array('shiptrack_agent'));
        $assigned_agent = $shipment_id ? get_post_meta($shipment_id, 'assigned_agent', true) : '';
        WPSTForm::gen_field(array(
            'key' => 'assigned_agent',
            'type' => 'select',
            'label' => __('Agent', 'shiptrack'),
            'class' => 'selectize w-100',
            'options' => $agents,
            'description' => !empty($agents) ? '' : '<span class="text-danger">'.esc_html__('Create shiptrack agent user.', 'wpst').'<a href="user-new.php" target="_blank"> '.esc_html('here', 'wpst').' <i class="fa fa-external-link"></i></a></span>',
    ...
    {
        $access_roles_assign = wpst_get_roles_can_assign();
        if (!wpst_is_user_admin() && !in_array('shiptrack_editor', $access_roles_assign)) {
            return false;
        }
        $editors = wpst_get_users_list(array('shiptrack_editor'));
        $assigned_editor = $shipment_id ? get_post_meta($shipment_id, 'assigned_editor', true) : '';
        WPSTForm::gen_field(array(
            'key' => 'assigned_editor',
            'type' => 'select',
            'label' => __('Editor', 'shiptrack'),
            'class' => 'selectize w-100',
            'options' => $editors,
